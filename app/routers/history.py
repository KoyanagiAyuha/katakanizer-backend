...
from ..database import get_db, ConversionHistory, LineMapping as DBLineMapping, User, Favorite
...
def get_my_conversion_history(
    ...
    # お気に入りのIDを取得
    favorite_ids = set(
        db.query(Favorite.conversion_id).filter(
...
def search_conversion_history(
    ...
    search_query = db.query(ConversionHistory).outerjoin(
        User, ConversionHistory.user_id == User.id
    ).filter(ConversionHistory.is_public == True)
